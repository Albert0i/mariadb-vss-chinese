...
#### III. Create a Lexer Preference (Preferable)
...
DROP INDEX albertoi.ft_textChiSeg; 

CREATE INDEX albertoi.ft_textChiSeg ON documents(textChiSeg)
  INDEXTYPE IS CTXSYS.CONTEXT
  PARAMETERS('LEXER segmented_lexer');
--  

INSERT INTO albertoi.documents (textChi, textChiSeg) VALUES
...
SELECT count(*) FROM albertoi.documents;
                    
SELECT * FROM albertoi.documents 
WHERE CONTAINS(TEXTCHISEG, '法國' ) > 0; 

SELECT * FROM albertoi.documents 
WHERE CONTAINS(TEXTCHISEG, '東西' ) > 0;       
...
[Querying with Oracle Text](https://docs.oracle.com/en/database/oracle/oracle-database/19/ccapp/querying-with-oracle-text.html#GUID-E330ED1C-CEEF-4433-B0A2-7ACB39763053)
